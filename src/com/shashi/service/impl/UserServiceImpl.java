...
public class UserServiceImpl implements UserService {
	...
	public UserServiceImpl(UserRole userRole) {
		// UserRole is ignored; always use 'users' table for all user types
	}
	...
	public String registerUser(UserBean customer) {
		String responseCode = ResponseCode.FAILURE.toString();
		String query = "INSERT INTO " + TABLE_NAME + " (fname, lname, mailid, pword, address, role, phno) VALUES (?, ?, ?, ?, ?, ?, ?)";
		try {
			Connection con = DBUtil.getConnection();
			PreparedStatement ps = con.prepareStatement(query);
			ps.setString(1, customer.getFName());
			...
			ps.setString(4, customer.getPWord());
			ps.setString(5, customer.getAddr());
			ps.setString(6, "CUSTOMER");
			ps.setLong(7, customer.getPhNo());
